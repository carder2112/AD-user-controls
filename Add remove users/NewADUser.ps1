...
#Regional Variables
    #Home Drive Server '\\server\homeshare
    $homesvr = "\\corpfps1\home"


#Ticket#
[void][Reflection.Assembly]::LoadWithPartialName('Microsoft.VisualBasic')
$title = 'User Creation Ticket'
    $msg   = 'Enter Ticket Number For User Creation:'
    $ticket = [Microsoft.VisualBasic.Interaction]::InputBox($msg, $title)

#New User Variable(s)
[void][Reflection.Assembly]::LoadWithPartialName('Microsoft.VisualBasic')
$title = 'New User'
    $msg   = 'Enter User Email Address. Must be in the format of Firstname.Lastname@domain.com'
    $user = [Microsoft.VisualBasic.Interaction]::InputBox($msg, $title)
    $userarray =$user.split("@")
    $name = $userarray[0]
    $dnamearray =$name.split(".")
    $gn = $dnamearray[0]
    $sn = $dnamearray[1]

#Validate Correct Naming format
        if ($name -notmatch "\.") {
...
[void][Reflection.Assembly]::LoadWithPartialName('Microsoft.VisualBasic')
$title = ''
    $msg   = 'Enter 2 Digit Country Code:'
    $country = [Microsoft.VisualBasic.Interaction]::InputBox($msg, $title)


#Description
[void][Reflection.Assembly]::LoadWithPartialName('Microsoft.VisualBasic')
$title = ''
    $msg   = 'Enter User Description:'
    $desc = [Microsoft.VisualBasic.Interaction]::InputBox($msg, $title)
#Company
[void][Reflection.Assembly]::LoadWithPartialName('Microsoft.VisualBasic')
$title = ''
    $msg   = 'Enter Company Name:'
    $company = [Microsoft.VisualBasic.Interaction]::InputBox($msg, $title)
#Office
[void][Reflection.Assembly]::LoadWithPartialName('Microsoft.VisualBasic')
$title = ''
    $msg   = 'Enter Office City:'
    $office = [Microsoft.VisualBasic.Interaction]::InputBox($msg, $title)


#User to Copy From Variable
[void][Reflection.Assembly]::LoadWithPartialName('Microsoft.VisualBasic')
$title = 'User to Copy from'
    $msg   = 'Enter User to Copy From UPN/Email:'
    $copy = [Microsoft.VisualBasic.Interaction]::InputBox($msg, $title)

#Prompt to complete User Creation
cls
Write-Host "Continue Creating new User with the following attributes?"
Write-Host "Ticket Number: " $ticket
Write-Host "First Name:    " $gn
Write-Host "Last Name:     " $sn
Write-Host "Email Address: " $user
Write-Host "Login ID:      " $user
Write-Host "Description:   " $desc
Write-Host "Company:       " $company
Write-Host "Office:        " $office
Write-Host "Copy of User:  " $copy
Write-Warning "Continue" -WarningAction Inquire


#Build User Account
#Add this to below to create Home Directories "-HomeDirectory $homesvr"\"$name `"
New-ADUser -Name $gn" "$sn `
            -DisplayName $gn" "$sn `
            -GivenName $gn `
            -Surname $sn `
            -Country $country `
...
Write-Host "Building Account! -- Please Wait!!!"
Start-Sleep -Seconds 25

#Set SMTP Addresses
Get-ADUser $name -Properties * | Set-ADUser -replace @{mailNickname=$name}
    $sm1 = "SMTP:"
    $sm1 = $sm1.ToUpper()
    $sm2 = "smtp:"
    $sm2 = $sm2.ToLower()
    $rmoms = "@rpmpcg.mail.onmicrosoft.com"
    $roms = "@rpmpcg.onmicrosoft.com"
Get-ADUser $name -Properties * | Set-ADUser -add @{ProxyAddresses="$sm1$user"}
Get-ADUser $name -Properties * | Set-ADUser -add @{ProxyAddresses="$sm2$user$roms"}
# Get-ADUser $name -Properties * | Set-ADUser -add @{targetAddress="$sm1$rmoms"}

Write-Host "Adding Group Memberships! -- Please Wait!!!"
Start-Sleep -Seconds 10

#Copy Groups

$cf = Get-ADUser -Filter "UserPrincipalName -eq '$($copy)'" -Properties MemberOf
$ct = Get-ADUser -Filter "UserPrincipalName -eq '$($user)'" -Properties MemberOf
$cf.memberof | Where{$ct.MemberOf -notcontains $_} | Add-ADGroupMember -Members $ct -ErrorAction SilentlyContinue

#Select User Target OU
...
$pre = "Ticket for User Creation: #"
$setticket = $pre+$ticket
Get-ADuser -Identity $name | Set-ADUser -Add @{info =  $setticket }
Get-ADuser -Identity $name | Set-ADUser -Add @{Description = $Desc}
Get-ADuser -Identity $name | Set-ADUser -Add @{physicalDeliveryOfficeName = $Office}
Get-ADuser -Identity $name | Set-ADUser -Add @{Company = $Company}


#Move User to selected OU
$pick = Choose-ADOrganizationalUnit
    Get-ADUser $name -Properties DistinguishedName | Move-ADObject -TargetPath $pick.DistinguishedName